...
class Data_exchanger(object):
    ...
    def __init__(self, user, dsn, base_table):
        ...
        if base_table == "MATCH" or base_table == "FEATURE_MATCH":
            ...
            print("base_table of 'MATCH' or 'FEATURE_MATCH' expected in production, but found '{0}' instead".format(
                base_table))
            sys.exit(1)
    ...
    def check_dbcodes(self, dbcodes):
        ...
        for dbcode in dbcodes:
            ...
            if query_result != dbcode:
                print("Error: Base table '{0}' and DB code '{1}' failed validation due to query '{2}'". format(
                    self.base_table, dbcode, query))
                print("Aborting!")
                sys.exit(1)

            if self.base_table == 'MATCH' and dbcode in ('g', 'j', 'n', 'q', 's', 'v', 'x'):
                print("Error: Match base table '{0}' incompatible with (feature match) database code '{1}'". format(
                    self.base_table, dbcode))
                print("Aborting!")
                sys.exit(1)
            elif self.base_table == 'FEATURE_MATCH' and dbcode not in ('g', 'j', 'n', 'q', 's', 'v', 'x'):
                print("Error: Feature match base table '{0}' incompatible with (match) database code '{1}'". format(
                    self.base_table, dbcode))
                print("Aborting!")
                sys.exit(1)
    ...
    def exchange_partition(self, partitioned_tbl, dbcode):
        ...
        try:
            self.cursor.execute(query)
            print("{0} exchanged with {1}_NEW.".format(
                partitioned_tbl, self.base_table))
        except cx_Oracle.DatabaseError as exception:
            print('Failed to execute ' + query)
            print(exception)
            exit(1)
    ...
    def rebuild_all_index(self, table_name):
        unusable_index = """SELECT index_name FROM user_indexes WHERE table_name = '{0}'
                            AND status = 'UNUSABLE'""".format(table_name)
    ...
    def reenable_all_constraint(self, table_name):
        disabled_constraint = """SELECT constraint_name FROM user_constraints WHERE table_name = '{0}'
                                 AND status = 'DISABLED'""".format(table_name)
        enable_constraint = "ALTER TABLE {0} ENABLE CONSTRAINT ".format(
            table_name)
    ...
    def rebuild_partition(self, table_name):
        unusable_partition = """SELECT p.index_name, partition_name FROM user_indexes i, user_ind_partitions p
                                ...
                                AND p.status = 'UNUSABLE'""".format(table_name)
...
def exchange_data(user: str, dsn: str, memberdb: list, base_table: str):
    ...
    print("Please compare the counts in this report to the one of {0}_TMP from CREATE_{0}_TMP.OUT to confirm this.".format(
        base_table))
