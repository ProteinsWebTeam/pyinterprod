...
def generate_old_report(cur, dbcode, outdir):
    ...
    sqlqueries = {
        ...
        "SELECT count(1) FROM interpro.method WHERE dbcode = '{}'",
        "Check how many of them skip_flagged and checked as Yes:":
       ...
       AND e.checked = 'Y'
       AND m.dbcode = '{}'""",
        "Number of integrated signatures:":
        """SELECT count(1)
       FROM interpro.method m
       ,interpro.entry2method e2m
       ...
       AND m.dbcode = '{0}'""",
        ...
        "SELECT count(1) FROM interpro.match WHERE dbcode = '{}'"}
...
def generate_new_report(cur, dbcode, outdir):
    ...
    sqlqueries = {
       ...
       FROM interpro.method
       WHERE dbcode = '{0}'""",
        "Name Change\nMethod ac	Old name	New name":
        """SELECT m.method_ac, m.name, s.name
       ...
       AND m.name != s.name""",
        "Description Change\nMethod ac	Old name	New name":
        """SELECT m.method_ac, m.description, s.description
       FROM interpro.method_stg s
       ,interpro.method m
       WHERE m.dbcode = '{0}'
       AND m.dbcode = s.dbcode
       ...
       AND m.description != s.description""",
        ...
        """SELECT m.method_ac, m.sig_type, s.sig_type
       FROM interpro.method_stg s
       ,interpro.method m
       WHERE m.dbcode = '{0}'
       AND m.dbcode = s.dbcode
       ...
       AND m.sig_type != s.sig_type""",
        "List of SignatureLess Entries as a Result of Deletion":
        """SELECT entry_ac,method_ac FROM (
       SELECT e2m.entry_ac
       ,e2m.method_ac
       ,CASE
       ...
       AND e2m.entry_ac = a.entry_ac
        ...
        "Number Matches Affected As a Result Of Deletion":
        """SELECT count(1)
       FROM interpro.match
