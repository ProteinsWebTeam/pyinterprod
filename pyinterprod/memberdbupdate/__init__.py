...
def main():
    ...
    parser.add_argument(
        "--resume", action="store_true", default=False, help="skip completed tasks (default: off)",
    )
    parser.add_argument(
        "--submit",
        action="store_const",
    ...
    tasks = [
        ...
        Task(
            ...
            fn=pronto.run,
            args=(args.config,),
            kwargs=dict(raise_on_error=True, tasks=["copy"]),
            scheduler=dict(queue=queue, mem=500),
            requires=["pronto"],
        ),
        Task(
